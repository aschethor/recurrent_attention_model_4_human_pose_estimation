...
def plot_3Dpose_simple(ax, pose_3d, bones, radius=10, colormap='gist_rainbow', color_order=[0, 5, 9, 15, 2, 10, 12, 4, 14, 13, 11, 3, 7, 8, 6, 1], plot_handles=None):
    ...
    if not plot_handles is None:
        for i, bone in enumerate(bones):
            plot_handles[i].set_data(XYZ[0:2, bone])
            plot_handles[i].set_3d_properties(XYZ[2,bone])
    else:
        ...
        for i, bone in enumerate(bones):
            ...
            colorIndex = (color_order[i] * cmap.N / float(maximum))
            color = cmap(int(colorIndex))
            line_handle = ax.plot(XYZ[0,bone], XYZ[1,bone], XYZ[2,bone], color=color)
            plot_handles.extend(line_handle) #for whatever reason plot already returns a list
...
def plot_3Dpose(ax, pose_3d, bones, radius=10, colormap='gist_rainbow', color_order=[0, 5, 9, 15, 2, 10, 12, 4, 14, 13, 11, 3, 7, 8, 6, 1], set_limits=True, flip_yz=True, fixed_color = False):
    pose_3d = np.reshape(pose_3d, (3, -1))
...
#     proj3d.persp_transformation = orthogonal_proj
    ...
    #print(bones_cat)
    plot_3Dpose(ax, pose_3d_cat.T, bones_cat, radius=radius, colormap=colormap, color_order=color_order_cat)
    ax.tick_params(
